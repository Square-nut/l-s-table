            ...
            <hos-row>
                ...
                <!-- <hos-col :span="12" v-has-permi="{key: status+'base:perm:role:name',formRule: rules,elModel:'type'}">
					<hos-form-item :label="$t('common.type')" prop="type">
						<hos-select v-model="dialogForm.type" placeholder="请选择" :disabled="$m.isDisabled(status+'base:perm:role:type')">
    ...
    methods: {
        ...
        getImageFile(file, fileList) {
            ...
            if (type.indexOf(fileType) === -1) {
                ...
                fileList.splice(-1, 1);
                this.$message.error(this.$t("role.uploadno")); //this.imageUrl = ""; //移除选中图片
                return false;
            } else {
                isAccept = true;
            }
            if (!isLt2M) {
                fileList.splice(-1, 1); //this.imageUrl = ""; //移除选中图片
            ...
            if (isLt2M == true && isAccept == true) {
                ...
                return false;
            } //this.beforeAvatarUpload(file); // 大于1张隐藏
